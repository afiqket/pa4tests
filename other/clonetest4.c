...
main(int argc, char *argv[])
{
    ...
    printf(1, "[clonetest4: Child thread can call clone, but they share the same thread parent]\n");
    ...
    if (tid1 == 0) {
        ...
        if (tid2 < 0) {
            sleep(50);
            printf(1, "Thread B: clone failed\n");
            free(buf);
            exit();
        }
        if (tid2 == 0) {
            ...
            sleep(100);
            printf(1, "Thread C: Hello world!\n");
            exit(); 
        }
        sleep(50);
        printf(1, "Thread B: Created thread C (tid = %d, pid = %d)\n", tid2, getpid());
        exit(); 
    }

    // 3) Back in the parent: clone() returned child’s TID
    sleep(10);
    printf(1, "Parent: Created thread B (tid = %d, pid = %d)\n", tid1, getpid());
    ...
    if (jtid1 < 0) {
        ...
        printf(1, "Parent: successfully joined thread B (tid = %d)\n", jtid1);
    }
    ...
    if (jtid2 < 0) {
        ...
        printf(1, "Parent: successfully joined thread C (tid = %d)\n", jtid2);
    }

    printf(1, "Test done. There should not be any zombies or crashes\n", jtid2);
